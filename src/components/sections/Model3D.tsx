...
// Renderizzata solo se l'aereo ha un modello associato.
export default function Model3D({ aircraft }: { aircraft: Aircraft }) {
  if (!aircraft.model3d) return null;
      ...
      <div className="relative z-[1] mx-auto max-w-[1180px] px-6">
        <Reveal>
          <SectionHeader
            eyebrow="Vista 3D"
            title="Il modello"
            text="Modello tridimensionale del velivolo, in rotazione automatica sul proprio asse."
          />
        </Reveal>
        ...
        <Reveal>
          <div className="relative h-[clamp(420px,62vh,760px)] overflow-hidden rounded-[18px] border border-line bg-gradient-to-b from-panel to-bg2">
            <ModelViewer
              modelUrl={aircraft.model3d}
              poster={aircraft.thumb}
              posterAlt={aircraft.thumbAlt}
            />
          </div>
        </Reveal>
      </div>
